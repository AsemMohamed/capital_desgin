        ...
        @foreach($beds as $bed)
            <div class="isotope-item blog-item">
                <div class="blog-media">
                    <a href="{{url('album').'/' . $bed->id}}" class="thumb-overlay">
                        <img src="{{asset('images')}}/{{ $bed->imag }}" alt="{{ $bed->seo }}">
...
{{$images->id}}
                ...
                <div class="blog-desc align-center">
                    <div class="blog-headline">
                        <h6 class="post-category uppercase">{{ $bed->type }}</h6>
                        <h6>Height: {{ $bed->height }}<br>
                            Length: {{ $bed->length }}<br>
                            Width:  {{ $bed->width }}</h6>
                        <h5 class="post-name"><a href="blog-single-custom1.html"><strong>{{ $bed->name }}</strong></a></h5>
                        <h5 class="post-date"  style="color: green">{{ $bed->price }} L.E</h5>
                    </div>
                    <ul class="blog-meta">
