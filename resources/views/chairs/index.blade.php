...
@section('chair')
        ...
        <table class="table table-bordered bordered-bold table-striped text-center">
            ...
            @foreach($chairs as $chair)
                <tr>
                    <td>{{$chair->id}}</td>
                    <td>{{$chair->name}}</td>
                    <th width="2"><img src="{{url('/')}}/images/{{ $chair->image }}" class="img-thumbnail"></th>
                    <td>{{$chair->price}} L.E</td>
                   {{-- <td>{{$chair->url}}</td>--}}
                    <td>{{$chair->type}}</td>
                    ...
                    <td>{{$chair->length}}</td>
                    <td>{{$chair->width}}</td>
                    <td>{{$chair->created_at->diffForHumans()}}</td>
                    <td>{{$chair->updated_at->diffForHumans()}}</td>
                    <th style="" class="inline-block">
                        <form style="text-align: center;"  action="{{route('chairs.destroy', $chair->id)}}" method="post">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}

                            <a href="{{route('chairs.edit', $chair->id)}}" class="btn btn-warning">Edit</a>
    ...
    {{--{!! $users->links() !!}--}}
    <div align="center">
        <a href="{{route('chairs.create')}}" class="btn btn-success btn-lg">add</a>
    </div>
    <br>
@endsection